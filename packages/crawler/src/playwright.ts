...
import { CrawlConfig, LiveCrawlData, ScreenshotRef, RecordingRef, LighthouseScores, HarLog, DomSnapshot, PerformanceMetrics, CtaInfo, FormInfo } from './types';
...
export class PlaywrightCrawler {
  ...
  private async createContext(): Promise<BrowserContext> {
    ...
    if (this.config.auth?.type === 'cookies' && this.config.auth.cookies?.length) {
  ...
  private async crawlPage(
    ...
    if (this.config.captureNetwork) {
      page.on('response', (response: Response) => {
        // Playwright Response doesn't have timing() method
        networkLogs.push({
          url: response.url(),
  ...
  private async runLighthouse(url: string): Promise<{ performance: number; accessibility: number; bestPractices: number; seo: number; pwa: number }> {
    try {
  ...
  private async extractPerformance(url: string): Promise<{ ttfb: number; fcp: number; lcp: number; tbt: number; cls: number }> {
